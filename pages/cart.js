...
function CartScreen() {
  ...
  return (
    <Layout title="Carrito de Compras">
      ...
      {cartItems.length === 0 ? (
        <div>
          Tu Carrito esta Vacio. <Link href="/">Ir de Compras</Link>
        </div>
      ) : (
        <div className="grid md:grid-cols4 md:gap-5">
            ...
            <table className="min-w-full">
              <thead className="border-b">
                <tr>
                  ...
                  <th className="p-5">Eliminar del carrito</th>
                </tr>
              </thead>
              <tbody>
                {cartItems.map((item) => (
                  <tr key={item.slug} className="border-b">
                    ...
                    <td className="p-5 text-center">
                      <button onClick={() => removeItemHandler(item)}>
                        <XCircleIcon className="h5 w-5"></XCircleIcon>
                      </button>
                    </td>
                  </tr>
                ))}
